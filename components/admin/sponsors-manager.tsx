  ...
  if (isLoading) {
    return (
      <div className="py-24 text-center">
        ...
        <p className="text-lg font-semibold text-slate-800">Loading sponsors...</p>
      </div>
    )
  }
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-navy-deep/80 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg overflow-hidden flex flex-col max-h-[90vh]">
            ...
            <form onSubmit={handleSave} className="overflow-y-auto p-6 space-y-6">
              ...
              <div className="space-y-2">
                <label className="text-xs font-bold uppercase text-slate-500">Logo Image {websiteUrl ? '(Optional)' : '*'}</label>
                <input 
                  type="file" 
                  accept="image/*"
                  onChange={e => {
